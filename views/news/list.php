...
//        ?>
        ...
        <div style="min-height: 300px;">
            ...
            <?php if (isset($data['tags'])): ?>
                ...
                <?php endforeach; ?>
            <?php endif; ?>
            <div style="text-align: right;">Количество просмотров: <?= $data['cnt_visit']; ?> Читают:<span id="read"></span></div>
            <h3> Messages: <span class="badge"><?=$data['comments']['count'];  ?></span></h3>

            <?php if (Session::get('login')) : ?>
                <form method="post" action="">
                    <div class="form-group">
                       <textarea rows="3" placeholder="Написать комментарий...." name="comment" class="form-control"></textarea>
                    </div>
                    <button type="submit" name="submit" class="btn btn-info btn-sm"><a href="#"></a> Добавить коммент</button>
                        ...
                        <div class="panel-footer" style="padding: 4px 15px; overflow: hidden;"  >
                            ...
                            <div style="float: right;">
                                ...
                                <button type="button" id="like" class="btn btn-default btn-xs">Like:
                                    <span class="glyphicon glyphicon-thumbs-up" aria-hidden="true"><?= $value['cnt_like'] ?></span>
                                </button>
                                <button type="button" id="dislike" class="btn btn-default btn-xs">Dislike:
                                    <span class="glyphicon glyphicon-thumbs-down" aria-hidden="true"><?= $value['cnt_dislike'] ?></span>
                                </button>
                            </div>
                        </div>
                    </div>
